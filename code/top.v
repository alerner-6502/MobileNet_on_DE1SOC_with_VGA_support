...
reg buffer_rst;
always @ (*) begin
	if(address_rst) buffer_rst = 1'b1;
	if(rd_addr == 320*169+267) buffer_rst = 1'b0;
end
...
hellosoc_top TFT(
	...
	.vga_g         ( vga_g ),
	.vga_b         ( vga_b ),
	.frame_rst    ( address_rst ),
	.buffer_rst   ( buffer_rst )
);

reg [9:0] x_sdram;

//wire frame_reset = address_rst && wr_enable;

always @(posedge rd_ready or posedge address_rst)
begin
	if (address_rst)
	begin
		//rd_addr = 320*201+3;
...
/* assign end_gray = 1'b0;
assign output_data = 11'd0;

assign end_neiroset = 1'b0;
assign RESULT = 4'd0; */
